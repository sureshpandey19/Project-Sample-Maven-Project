...
echo "getting star domain missing list....."
...
list=$(echo "select clipid from webfixlistviewer where clipid NOT IN (SELECT clipid FROM mamdatalist WHERE deviceuid IN ('4b576a44-eeda-11eb-98ed-0cc47aaa5f96','4b593031-eeda-11eb-98ed-0cc47aaa5f96','4b599274-eeda-11eb-98ed-0cc47aaa5f96')) union SELECT * FROM (SELECT clipid FROM webplaylistviewer  WHERE playlistdate >= CURDATE() AND clipid NOT IN (SELECT clipid FROM mamdatalist WHERE deviceuid IN ('4b576a44-eeda-11eb-98ed-0cc47aaa5f96','4b593031-eeda-11eb-98ed-0cc47aaa5f96','4b599274-eeda-11eb-98ed-0cc47aaa5f96')) AND eventtype = 'PRI'  AND channelid IN (SELECT uid FROM channels ) group by clipid) AS T" | mysql -h $DbHost_cloud -P $DbPort_cloud -u $DbUser_cloud -p$DbPasswd_cloud ${DbName_cloud}) 2> /dev/null
